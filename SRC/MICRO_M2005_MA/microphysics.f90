...
use params, only: rgas, rv, cp, lcond, lsub, fac_cond, fac_sub, ggr, pi, &
...
use grid, only: nx,ny,nzm,nz, &  !grid dimensions; nzm = nz-1 # of scalar lvls
     ...
     doscamiopdata, isinitialized_scamiopdata
...
real, allocatable, dimension(:,:,:) :: massdryfrac   ! fraction of total aerosol mass not in-cloud
...
!     donudging_aerosol, &  ! Enable nudging of aerosol
      ...
      acttransferoption, & !  flavor of aitken transfer scheme to use
      ...
      do_rescale_aerosol_on_branch_restart, &
      Naccum_scale_factor_branch_restart, &
      Qaccum_scale_factor_branch_restart, &
      Naitken_scale_factor_branch_restart, &
      Qaitken_scale_factor_branch_restart
!bloss: add later??
!bloss      do_micro3dout         ! flag to output 3d snapshot for a bunch of micro tendencies
  ...
  if(ABS(massfactor-1.).gt.4*EPSILON(1.)) then
   ...
   ! intialize # of microphysics outputs to zero
   nfields2D_micro = 0
   nfields3D_micro = 0
   
   ! Account for additions to 2D and 3D outputs
   nfields2D_micro = nfields2D_micro + 5  ! for DMS, qad, qad2, nad, nad2 surface fluxes
   if(dopredictNc) nfields2D_micro = nfields2D_micro + 3 ! for vertically-integrated QC, QC*NC, and optical depth
   if(doprecip) nfields2D_micro = nfields2D_micro + 2 ! for vertically-integrated QR and QR*NR
  ...
  if(doprogaerosol) then
     nmicro_proc = nmicro_proc &
          + 32 & ! add aerosol tendencies (mostly limiters) ! need to recalculate this FIX
          + 6 ! QAD/NAK/QAK limiters related to minimum aerosol number/size
...
subroutine micro_init()
  ...
  real :: naccum_new !bloss: Aerosol rescaling w/nrestart=2
  integer :: status
...
!!$  end if
  ...
  !------------------------------------------------------------------
  !bloss(2023-11): Initialize mk_ref fields, used for aerosol N/q from
  !   netcdf forcing files, outside the if(nrestart.eq.0) statement.
  !   This makes the behavior of those fields work properly in both
  !   fresh and restarted runs.
  if(doprogaerosol.AND.(aerinitmode.eq.0)) then
     ! set up reference profiles for nudging

     ! If necessary, get forcings from soundings in SCAM IOP forcings
     if(.NOT.allocated(AccumAerosolNumber_snd)) then
        if(doscamiopdata) then
           ...
           write(*,*) 'Error in MICRO_M2005_MA: initial conditions for aerosols'
           write(*,*) '  when using aerinitmode==0 should be specified in '
           write(*,*) '  IOP netcdf file using the variables: Na_accum, qa_accun,.'
           write(*,*) '  Na_aitken and qa_aitken.'
           write(*,*) '********* Model stopping ... *************'
           call task_abort()
        end if

        if (MINVAL(AccumAerosolNumber_snd).lt.-1000.) then
           write(*,*) 'Na_accum not available.'
           status = -1
        end if
        if (MINVAL(AccumAerosolMass_snd).lt.-1000.) then
             write(*,*) 'qa_accum not available.'
           status = -1
        end if
        if (MINVAL(AitkenAerosolNumber_snd).lt.-1000.) then
             write(*,*) 'Na_aitken not available.'
           status = -1
        end if
        if (MINVAL(AitkenAerosolMass_snd).lt.-1000.) then
             write(*,*) 'Na_aitken not available.'
           status = -1
        end if

        if(status.ne.0) then
           write(*,*) 'Cannot get aerosols soundings.  Stopping...'
           call task_abort()
        end if
     ...
     if(masterproc) then
        ...
        write(*,*) '  the reference aerosol sounding for nudging are initialized '
        write(*,*) '  using soundings from the IOP netcdf forcing file.  '
        write(*,*) '  Initial profile below ...'
        do k = 1,nzm
           write(*,845) z(k), pres(k), Na_accum(k), qa_accum(k), &
                ( qa_accum(k)/Na_accum(k) / EXP(9.*(log(sigma_accum)**2)/2.) &
                / ( rho_aerosol*pi/6. ) )**(1./3.), &
...
845        format('Aerosol: z, pres, Nacc, qacc, Dacc, Nait, qait, Dait = ', F10.2,7E14.6)
     ...
     mk_ref(:, iqad) = qa_accum*massfactor
     mk_ref(:, inad2) = Na_aitken
     mk_ref(:, iqad2) = qa_aitken*massfactor

     if(do_rescale_aerosol_on_branch_restart) then
        !rescale the reference aerosol profiles during a branch restart

        ! first, aitken mode
        mk_ref(:,inad2) = Naitken_scale_factor_branch_restart*mk_ref(:,inad2)
        mk_ref(:,iqad2) = Qaitken_scale_factor_branch_restart*mk_ref(:,iqad2)

        ! second, accumulation mode mass, which is all held in iqad
        mk_ref(:,iqad) = Qaccum_scale_factor_branch_restart*mk_ref(:,iqad)
        
        ! last, accumulation mode number, which here represents both
        !   cloud and unactivated aerosol.
        mk_ref(:,inad) = Naccum_scale_factor_branch_restart*mk_ref(:,inad)
     end if

  end if !doprogaerosol.AND.(aerinitmode.eq.0)
...
!!$       dumM2 = (POCperturb_Na)*RHOA*pi/6.*(aer_rm1**3)*EXP(9.*(log(aer_sig1)**2)/2.) 
     ...
     if(doprogaerosol.AND.do_rescale_aerosol_on_branch_restart) then
        !rescale the aerosol fields during a branch restart
        ! first, aitken mode
        micro_field(:,:,:,inad2) = &
             Naitken_scale_factor_branch_restart*micro_field(:,:,:,inad2)
        micro_field(:,:,:,iqad2) = &
             Qaitken_scale_factor_branch_restart*micro_field(:,:,:,iqad2)

        ! second, accumulation mode mass, which is all held in iqad
        micro_field(:,:,:,iqad) = &
             Qaccum_scale_factor_branch_restart*micro_field(:,:,:,iqad)
        
        ! last, accumulation mode number, which is a bit more complicated
        !   because it is split between inad and incl.  Here, decrease the
        !   total by the specified scale factor and set Nc = MIN(Nc_old,Naccum_new)
        do k = 1,nzm
           do j = 1,ny
              do i = 1,nx
                 naccum_new = Naccum_scale_factor_branch_restart &
                      *(micro_field(i,j,k,inad) + micro_field(i,j,k,incl))
                 micro_field(i,j,k,incl) = &
                      MIN(micro_field(i,j,k,incl), naccum_new)
                 micro_field(i,j,k,inad) = &
                      MAX(0., naccum_new - micro_field(i,j,k,incl))
              end do
           end do
        end do
     end if
...
real :: ddry
real :: sqrt2, xx, maerofrac, naerofrac, mdryfrac, ndryfrac, total_acc_number
...
if(debug) then
   do n = 1,nmicro_fields
      call micro_where_negative(n,'START of micro_proc')
   end do!
end if
...
do k = 1,nzm
   do j = 1,ny
      do i = 1,nx
         if (micro_field(i,j,k,incl).gt.0.1) then !bloss: Use Nc since Nad could be zero in-cloud.
            ! partition accumulation mode aerosol mass between cloud droplets and unactivated aerosol
            total_acc_number = micro_field(i,j,k,inad) + micro_field(i,j,k,incl)
            ndryfrac = micro_field(i,j,k,inad)/total_acc_number
            massdryfrac(i,j,k) = mass_fraction( ndryfrac, log(sigma_accum) )
         end if
      end do
   end do
end do
!  use micro_field(:,:,:,iqaw) as temporary holder of in cloud aerosol mass
micro_field(1:nx,1:ny,1:nzm,iqaw) = (1.-massdryfrac) * micro_field(1:nx,1:ny,1:nzm,iqad)
micro_field(1:nx,1:ny,1:nzm,iqad) = massdryfrac * micro_field(1:nx,1:ny,1:nzm,iqad)
...
if(debug) then
   do n = 1,nmicro_fields
      call micro_where_negative(n,'BEFORE uptake')
   end do!
end if
!print*, '+++BEFORE uptake'
...
if(debug) then
   do n = 1,nmicro_fields
      call micro_where_negative(n,'AFTER uptake')
   end do!
end if
!print*, '+++After uptake'
!call micro_printstats(iH2SO4)
...
if(debug) then
   do n = 1,nmicro_fields
      call micro_where_negative(n,'AFTER coag')
   end do!
end if
...
!      tmpw = 0.5*(w(i,j,1:nzm) + w(i,j,2:nz))  ! MK: changed for stretched grids 
      ...
      effi1d(:) = 75.
      do k = 1,nzm
         if (tmpncl(k).gt.0.1) then !bloss: Use Nc since Nad could be zero in-cloud.
            ! partition accumulation mode aerosol mass between cloud droplets and unactivated aerosol
            ndryfrac = tmpnad(k)/(tmpnad(k) + tmpncl(k))
            mdryfrac = mass_fraction( ndryfrac, log(sigma_accum) )
          ...
          if(((mod(nstep,nscv).EQ.0).AND.(icycle.EQ.1)).OR.((nstep.EQ.1).AND.(icycle.EQ.1))) then
            ...
            do k = 1,nzm
               ...
               if (tmpnad(k).gt.0.1) then
                  ...
                  maerofrac = mass_fraction( naerofrac, log(sigma_accum) )
...
if(debug) then
   do n = 1,nmicro_fields
      call micro_where_negative(n,'AFTER scavenging')
   end do!
end if
...
do n = 1,nmicro_fields
   if (any(isnan(micro_field(1:nx,1:ny,1:nzm,n)))) loc_nan = 1
   ...
   if (glb_nan(1).eq.1) then
      print *,"NaN detected!!! in '", mkname(n)
      call write_fields3D()
      call task_abort()
   end if
end do
...
if(debug) then
   do n = 1,nmicro_fields
      call micro_where_negative(n,'END of micro_proc')
   end do!
end if

call micro_where_negative(iqad,'END of micro_proc')
...
subroutine micro_where_negative(ifield,string)
   implicit none
   ...
   do k = 1,nzm
      do j = 1, ny
          do i = 1, nx
             if (micro_field(i,j,k,ifield).lt.-EPSILON(1.)) then
827             format(A,'Negative',A,': i,j,k,value = ',3I8,E16.8)
                write(*,827) string, mkname(ifield), i,j,k, micro_field(i,j,k,ifield)
             end if
          end do
       end do
   end do
...
if(incl.gt.0) then
   !bloss: add output for product of NC and QC
   !  Dividing the mean of NCQC by the mean of QC will give
   !  the cloud-mass-weighted cloud droplet number concentration
   call add_to_namelist(count,microcount,'NCQC', &
        'Product of cloud droplet number and mass mixing ratio', '#/cm3 g/kg',0)
end if

if(inr.gt.0) then
   !bloss: add output for product of NR and QR
   !  Dividing the mean of NRQR by the mean of QR will give
   !  the cloud-mass-weighted cloud droplet number concentration
   call add_to_namelist(count,microcount,'NRQR', &
        'Product of rain droplet number and mass mixing ratio', '#/cm3 g/kg',0)
end if
...
do n = 1,nmicro_fields
   ...
   if(flag_number(n).eq.1) then
      ...
      chemtend(1:nzm,n) = chemtend(1:nzm,n)*rho(:)
      coagtend(1:nzm,n) = coagtend(1:nzm,n)*rho(:)
      scavtend(1:nzm,n) = scavtend(1:nzm,n)*rho(:)
      !bloss: mkstor is computed separately and includes factor of rho
      !bloss mkstor(1:nzm,n) = mkstor(1:nzm,n)*rho(:)
   end if
...
if(incl.gt.0) then
   ...
   !  the cloud-mass-weighted cloud droplet number concentration
   do k = 1,nzm
      tr0(k) = SUM(cloudliq(1:nx,1:ny,k)*micro_field(1:nx,1:ny,k,incl)*rho(k))
   end do
   call hbuf_put('NCQC',tr0,factor_xy*mkoutputscale(iqcl)*mkoutputscale(incl))
end if

if(inr.gt.0) then
   !bloss: add output for product of NR and QR
   !  Dividing the mean of NRQR by the mean of QR will give
   !  the rain-mass-weighted droplet number concentration
   do k = 1,nzm
      tr0(k) = SUM(micro_field(1:nx,1:ny,k,iqr)*micro_field(1:nx,1:ny,k,inr)*rho(k))
   end do
   call hbuf_put('NCQC',tr0,factor_xy*mkoutputscale(iqr)*mkoutputscale(inr))
end if
...
subroutine micro_stat_2Dinit(ResetStorage)
  ...
  if(ResetStorage.eq.1) then
    ...
    !         store initial profiles for computation of storage terms in budgets
     ...
     do n = 1,nmicro_fields
        if(flag_number(n).eq.1) then
           mkstor(1:nzm,n) = mkstor(1:nzm,n)*rho(1:nzm)
        end if
     end do
  end if
...
subroutine micro_write_fields2D(nfields1)
  ...
  integer :: i, j, k
  real(4), dimension(nx,ny,1) :: dms_t, qad_t, qad2_t, nad_t, nad2_t, tmp
  real :: coef
  ...
  if(dopredictNc) then
     ...
     do k = 1,nzm
        do j=1,ny
           do i=1,nx
              ! following equation 5 in Wood Notes On Optical Depth Relationships
              !   dates 23 Aug 2006.
              tmp(i,j,1)=tmp(i,j,1)+dz*adz(k) &
                   *( 1.5*rho(k)*qcl(i,j,k) &
                   / (rho_water*1e-6*reffc(i,j,k)) )
           end do
        end do
     end do
     name='TAUQC'
     long_name='Optical Depth of Cloud Liquid  (vertically integrated)'
     units='1'
     call compress3D(tmp,nx,ny,1,name,long_name,units, &
          save2Dbin,dompi,rank,nsubdomains)
...
subroutine cloudradar_init()
  ...
  !bloss: This is done in micro_setparm().
  !bloss nfields3D_micro = nfields3D_micro + 1
