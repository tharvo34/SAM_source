...
!  USE module_state_description
  ! parameters from SAM and options from wrapper routine.
   ...
   use hoppel_transfer, only: mass_fraction, hoppel_aitken_accum_transfer, &
        hoppel_aitken_accum_transfer_W2022b
...
! AEROSOL PARAMETERS FOR IACT = 2
     ...
     REAL, PRIVATE ::      NA2     !Total number mode 1
     REAL, PRIVATE ::      QA2     !Total mass mode 1
...
! BRNR ADDED 2/13/12
     ...
     REAL, DIMENSION(KMS:KME) ::  NAD2POSLIM ! POS CHANGE NAD2 DUE TO LIMITERS
     REAL, DIMENSION(KMS:KME) ::  NAD2NEGLIM ! NEG CHANGE NAD2 DUE TO LIMITERS
     ...
     REAL, DIMENSION(KMS:KME) ::  QADPOSLIM ! POS CHANGE QAD DUE TO LIMITERS
     REAL, DIMENSION(KMS:KME) ::  QADNEGLIM ! NEG CHANGE QAD DUE TO LIMITERS
     REAL, DIMENSION(KMS:KME) ::  QAD2POSLIM ! POS CHANGE QAD2 DUE TO LIMITERS
     REAL, DIMENSION(KMS:KME) ::  QAD2NEGLIM ! NEG CHANGE QAD2 DUE TO LIMITERS
! HM ADDED 12/13/06
...
! INIT LIMITER PROFILES TO ZERO
            ...
            NADPOSLIM(KTS:KTE) = 0.
            NADNEGLIM(KTS:KTE) = 0.
            NAD2POSLIM(KTS:KTE) = 0.
            NAD2NEGLIM(KTS:KTE) = 0.
            QVPOSLIM(KTS:KTE) = 0. 
            QCNEGLIM(KTS:KTE) = 0. 
            QRNEGLIM(KTS:KTE) = 0. 
            QAWNEGLIM(KTS:KTE) = 0.
            ...
            QADPOSLIM(KTS:KTE) = 0.
            QADNEGLIM(KTS:KTE) = 0.
            QAD2POSLIM(KTS:KTE) = 0.
            QAD2NEGLIM(KTS:KTE) = 0.
...
! DROPLET ACTIVATION FROM ABDUL-RAZZAK AND GHAN (2000)
                              ...
                              IF ((NANEW1.GT.0.AND.RM1.GT.0).OR.(NANEW2.GT.0.AND.RM2.GT.0)) THEN
                                ...
                                D3 = 0.                                
                                 ...
                                 !bloss: Cap below transfer
!!$				 NCNEW = MIN(NA1, NCNEW)  ! cap activation could make this optional

                                 NTRANS = 0.
                                 QTRANS = 0.
                                 IF (doacttransfer) THEN
                                    select case (acttransferoption)
                                    case (0)
                                       ! Follow Wyant et al (2022, JAMES, sec. 2.1)
                                       !   If the Hoppel diameter (the crossover of Aitken-accumulation size
                                       !   distributions) is larger than the critical diameter, transfer 
                                       !   Aitken aerosols with size DC_aitken so that the Hoppel diameter 
                                       !   equals the critical diameter.
                                       call hoppel_aitken_accum_transfer(NAD2_3D(K), NA1, QAD2_3D(K), QA1, &
                                            SG2, SG1, DC2(K), DC1(K), RHOA, RHOA, NTRANS, QTRANS)

                                    case(1)
                                       ! Similar to above, but the size of transferred Aitken aerosols is 
                                       !   no smaller than the mass mean diameter of the Aitken mode.
                                       call hoppel_aitken_accum_transfer_W2022b(NAD2_3D(K), NA1, QAD2_3D(K), QA1, &
                                            SG2, SG1, DC2(K), DC1(K), RHOA, RHOA, NTRANS, QTRANS)

                                    case(2)
                                       ! Transfer mass of activated Aitken aerosols only when DC < D_hoppel
                                       !   (as defined above).
                                       NA2 = NANEW2/RHO(K)  ! units of /kg  total number of mode 2
                                       QA2 = MAER2/RHO(K)   ! units of kg/kg total mass of mode 2

                                       ! if the concentration of Aitken aerosols is larger at the critical diameter
                                       !   than that of the accumulation mode, then transfer the activated aitken mass
                                       !   to the accumulation mode
                                       eta1 = NA1*exp(-0.5 * (log(DG1(K)/DC1(K))/log(SIG1))**2)/(SQRT(2.*pi)*log(SIG1)) ! accum
                                       eta2 = NA2*exp(-0.5 * (log(DG2(K)/DC2(K))/log(SIG2))**2)/(SQRT(2.*pi)*log(SIG2)) ! aitken

                                       if(eta2.gt.eta1) then
!!$                                          NTRANS = NARG2(K)
                                          NTRANS = MAX(0., MIN( NACTDIFF(K), NARG2(K) ) )

                                          ! compute how much aitken mass is activated
                                          DUM2 = QA2/2.*(1.-DERF1(UU2-3.*SQRT(2.)/2.*LOG(SIG2)))
                                          QTRANS = MIN(QA2,DUM2) ! Make sure doesn't exceed available aerosol mass
                                       end if
				 ...
				 NCNEW = MIN(NA1, NCNEW)  ! cap activation
                                 ...
                                 QAWNEW = QA1 * (1. - mass_fraction( NADNEW/NA1, log(sigma_accum) ) )
      ...
      if(do_accumulate_micro_proc_rates) then
        ...
        if (IPRGAER.eq.1) then
          ...
          idx = idx + 1
           ...
           idx = idx + 1
           micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + QADPOSLIM  
           idx = idx + 1
           micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + QADNEGLIM
           idx = idx + 1
           micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NAD2POSLIM  
           idx = idx + 1
           micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NAD2NEGLIM
           idx = idx + 1
           micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + QAD2POSLIM  
           idx = idx + 1
           micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + QAD2NEGLIM
       end if   
        if(iliq.eq.0) then
          idx = idx + 1
