...
! brnr check thermal conductivity value above, look up value for SO2
  ...
  REAL(KIND=dp), DIMENSION(:,:,:,:), ALLOCATABLE :: &
       g_c_temp, & ! intermediate storage to save reallocation each call to scavenging coefficient
       g_r_temp   !   " " "
...
CONTAINS
  ...
  SUBROUTINE memory(cflag)
      ...
      ! ALLOCATE(xgl_g(ngl),wgl_g(ngl)); xgl_g=0._dp; wgl_g=0._dp
       ...
       ELSE
         ...
         ! ALLOCATE(e_g(na,ng,nt,np,1));     e_g=0._dp
          ...
          ALLOCATE(g_c_temp(na,nt,np,nrhw))
          ALLOCATE(g_r(na,npr,nt,np,1));    g_r=0._dp
          ALLOCATE(g_r_temp(na,nt,np,1))
    ...
    ELSEIF (TRIM(cflag) == 'deallocate') THEN
       ...
       IF (ALLOCATED(g_c_temp)) DEALLOCATE(g_c_temp)
       IF (ALLOCATED(g_r_temp)) DEALLOCATE(g_r_temp)
