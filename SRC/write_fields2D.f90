...
if(ocean_flux_option.eq.36) nfields = nfields + 1 ! output skin temperature
nfields1=0
...
if(ocean_flux_option.eq.36) then
   ...
   do j=1,ny
    do i=1,nx
      tmp(i,j,1)=tskinxy(i,j)
    end do
   end do
  name='TSKIN'
  long_name='Skin Temperature (with COARE3.6 Cool Skin Correction)'
  units='K'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
end if
