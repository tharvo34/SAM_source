subroutine rad_full()
  ...
  real(r4) tsurface !bloss: Holds SST or skin temperature
...
!bloss(2021-08): Use skin temperature predicted by COARE3.6 cool skin algorithm
!         if that scheme is used to compute surface fluxes
!bloss           lwupsfc(1) = stebol*(sstxy(1,1)+t00)**4 ! CGS units

           tsurface = sstxy(1,1)+t00
           IF(ocean_flux_option.eq.36) THEN
             ! Use skin temperature computed by COARE3.6 surface flux scheme
             tsurface = tskinxy(1,1)
           END IF
           lwupsfc(1) = stebol*tsurface**4 ! CGS units
...
!bloss           lwupsfc(1) = stebol*(sstxy(1,1)+t00)**4 ! CGS units
           ...
           tsurface = sstxy(i,j)+t00
           IF(ocean_flux_option.eq.36) THEN
             ! Use skin temperature computed by COARE3.6 surface flux scheme
             tsurface = tskinxy(i,j)
           END IF
           lwupsfc(1) = stebol*tsurface**4 ! CGS units
	      ...
	      coszrs = coszrs_in(1) ! needed for the isccp simulator
              ...
              !bloss(2021-08): Since this temperature is only used for deciding if
              !  the surface is liquid or ice, do not input the skin temperature
              !  from the COARE3.6 cool skin algorithm xhere.
