...
!+---+-----------------------------------------------------------------+
!
      MODULE module_mp_thompson_iso_AerosolAware

      USE module_wrf_error
      USE module_mp_radar
!SAM#if ( defined( DM_PARALLEL ) && ( ! defined( STUBMPI ) ) )
!SAM      USE module_dm, ONLY : wrf_dm_max_real
!SAM#endif
      USE module_dm
      USE module_model_constants, only : RE_QC_BG, RE_QI_BG, RE_QS_BG
      USE grid, only: dompi, masterproc !SAM
      USE micro_params, only: doicemicro, doaerosols, Nc0, & !SAM
           doRearrangeRainIceCollection, & !SAM
           nproc_rates_mass, nproc_rates_number !SAM


      IMPLICIT NONE

!SAM      LOGICAL, PARAMETER, PRIVATE:: iiwarm = .false.
...
!SAM      REAL, PARAMETER, PRIVATE:: Nt_c = 100.E6
...
!SAM      SUBROUTINE thompson_init(hgt, orho, nwfa2d, nwfa, nifa,           &
!SAM                          dx, dy, is_start,                             &
!SAM                          ids, ide, jds, jde, kds, kde,                 &
!SAM                          ims, ime, jms, jme, kms, kme,                 &
!SAM                          its, ite, jts, jte, kts, kte)

      IMPLICIT NONE

!SAM      INTEGER, INTENT(IN):: ids,ide, jds,jde, kds,kde, &
!SAM                            ims,ime, jms,jme, kms,kme, &
!SAM                            its,ite, jts,jte, kts,kte
!SAM      REAL, DIMENSION(ims:ime,kms:kme,jms:jme), INTENT(IN):: hgt

!..OPTIONAL variables that control application of aerosol-aware scheme

!SAM      REAL, DIMENSION(ims:ime,kms:kme,jms:jme), OPTIONAL, INTENT(INOUT) :: nwfa, nifa
!SAM      REAL, DIMENSION(ims:ime,jms:jme), OPTIONAL, INTENT(INOUT) :: nwfa2d
!SAM      REAL, DIMENSION(ims:ime,kms:kme,jms:jme), OPTIONAL, INTENT(IN) :: orho
!SAM      REAL, OPTIONAL, INTENT(IN) :: DX, DY
!SAM      LOGICAL, OPTIONAL, INTENT(IN) :: is_start
...
!SAM(bloss): In SAM, nwfa/nifa are initialized in micro_init().

!SAM      write(mp_debug,*) ' DEBUG  checking column of hgt ', its+1,jts+1
!SAM      CALL wrf_debug(250, mp_debug)
!SAM      do k = kts, kte
!SAM         write(mp_debug,*) ' DEBUGT  k, hgt = ', k, hgt(its+1,k,jts+1)
!SAM         CALL wrf_debug(250, mp_debug)
!SAM      enddo
!SAM
!SAM      if (PRESENT(nwfa2d) .AND. PRESENT(nwfa) .AND. PRESENT(nifa)) is_aerosol_aware = .TRUE.
!SAM
!SAM      if (is_aerosol_aware) then
!SAM
!SAM!..Check for existing aerosol data, both CCN and IN aerosols.  If missing
!SAM!.. fill in just a basic vertical profile, somewhat boundary-layer following.
!SAM
!SAM!SAM#if ( defined( DM_PARALLEL ) && ( ! defined( STUBMPI ) ) )
!SAM!SAM      max_test = wrf_dm_max_real ( MAXVAL(nwfa(its:ite-1,:,jts:jte-1)) )
!SAM!SAM#else
!SAM      max_test = MAXVAL ( nwfa(its:ite-1,:,jts:jte-1) )
!SAM!SAM#endif
!SAM
!SAM      if (max_test .lt. eps) then
!SAM         write(mp_debug,*) ' Apparently there are no initial CCN aerosols.'
!SAM         CALL wrf_debug(100, mp_debug)
!SAM         write(mp_debug,*) '   checked column at point (i,j) = ', its,jts
!SAM         CALL wrf_debug(100, mp_debug)
!SAM         do j = jts, min(jde-1,jte)
!SAM         do i = its, min(ide-1,ite)
!SAM            if (hgt(i,1,j).le.1000.0) then
!SAM               h_01 = 0.8
!SAM            elseif (hgt(i,1,j).ge.2500.0) then
!SAM               h_01 = 0.01
!SAM            else
!SAM               h_01 = 0.8*cos(hgt(i,1,j)*0.001 - 1.0)
!SAM            endif
!SAM            niCCN3 = -1.0*ALOG(naCCN1/naCCN0)/h_01
!SAM            nwfa(i,1,j) = naCCN1+naCCN0*exp(-((hgt(i,2,j)-hgt(i,1,j))/1000.)*niCCN3)
!SAM            airmass = 1./orho(i,1,j) * (hgt(i,2,j)-hgt(i,1,j))*dx*dy     ! kg
!SAM            nwfa2d(i,j) = nwfa(i,1,j) * 0.000196 * (airmass*2.E-10)
!SAM            do k = 2, kte
!SAM               nwfa(i,k,j) = naCCN1+naCCN0*exp(-((hgt(i,k,j)-hgt(i,1,j))/1000.)*niCCN3)
!SAM            enddo
!SAM         enddo
!SAM         enddo
!SAM      else
!SAM         has_CCN    = .TRUE.
!SAM         write(mp_debug,*) ' Apparently initial CCN aerosols are present.'
!SAM         CALL wrf_debug(100, mp_debug)
!SAM         write(mp_debug,*) '   column sum at point (i,j) = ', its,jts, SUM(nwfa(its,:,jts))
!SAM         CALL wrf_debug(100, mp_debug)
!SAM      endif
!SAM
!SAM
!SAM!SAM#if ( defined( DM_PARALLEL ) && ( ! defined( STUBMPI ) ) )
!SAM!SAM      max_test = wrf_dm_max_real ( MAXVAL(nifa(its:ite-1,:,jts:jte-1)) )
!SAM!SAM#else
!SAM      max_test = MAXVAL ( nifa(its:ite-1,:,jts:jte-1) )
!SAM!SAM#endif
!SAM
!SAM      if (max_test .lt. eps) then
!SAM         write(mp_debug,*) ' Apparently there are no initial IN aerosols.'
!SAM         CALL wrf_debug(100, mp_debug)
!SAM         write(mp_debug,*) '   checked column at point (i,j) = ', its,jts
!SAM         CALL wrf_debug(100, mp_debug)
!SAM         do j = jts, min(jde-1,jte)
...
!SAM               h_01 = 0.8*cos(hgt(i,1,j)*0.001 - 1.0)
!SAM            endif
!SAM            niIN3 = -1.0*ALOG(naIN1/naIN0)/h_01
!SAM            nifa(i,1,j) = naIN1+naIN0*exp(-((hgt(i,2,j)-hgt(i,1,j))/1000.)*niIN3)
!SAM            do k = 2, kte
!SAM               nifa(i,k,j) = naIN1+naIN0*exp(-((hgt(i,k,j)-hgt(i,1,j))/1000.)*niIN3)
!SAM            enddo
!SAM         enddo
!SAM         enddo
!SAM      else
!SAM         has_IN     = .TRUE.
!SAM         write(mp_debug,*) ' Apparently initial IN aerosols are present.'
!SAM         CALL wrf_debug(100, mp_debug)
!SAM         write(mp_debug,*) '   column sum at point (i,j) = ', its,jts, SUM(nifa(its,:,jts))
!SAM         CALL wrf_debug(100, mp_debug)
!SAM      endif
!SAM
!SAM      endif
...
!SAM#if ( WRF_CHEM == 1 )
!SAM      REAL, DIMENSION(ims:ime, kms:kme, jms:jme), INTENT(INOUT):: &
!SAM                          rainprod, evapprod
!SAM#endif
...
!SAM#if ( WRF_CHEM == 1 )
!SAM      REAL, DIMENSION(kts:kte):: &
!SAM                          rainprod1d, evapprod1d
!SAM#endif
      REAL, DIMENSION(its:ite, jts:jte):: pcp_ra, pcp_sn, pcp_gr, pcp_ic
      REAL:: dt, pptrain, pptsnow, pptgraul, pptice, pptcliq
      ...
      !bloss(SAM): extra arguments for mp_thompson
      REAL, DIMENSION(kts:kte) :: rho1d
      REAL, DIMENSION(kts:kte,6) :: mtendq, stendq
      REAL, DIMENSION(kts:kte,5) ::mtendn, stendn
      ...
      REAL, DIMENSION(kts:kte,1) :: proc_extra
      LOGICAL :: dosedimentation, do_accum_proc_rates, do_proc_extra
...
!     endif
      ...
      i_loop:  do i = i_start, i_end
         ...
         call mp_thompson(qv1d, qc1d, qi1d, qr1d, qs1d, qg1d, ni1d,     &
                      ...
                      pptrain, pptsnow, pptgraul, pptice, pptcliq, &
!SAM#if ( WRF_CHEM == 1 )
!SAM                      rainprod1d, evapprod1d, &
!SAM#endif
                      kts, kte, dt, i, j, &
                      rho1d, mtendq, mtendn, stendq, stendn, & !bloss: rho input, microphysical tendency outputs
                      mass_proc_rates, number_proc_rates, & !bloss: microphysical process outputs
                      dosedimentation,do_accum_proc_rates, & !bloss: flags for sedimentation, process rates
                      proc_extra, n_proc_extra, do_proc_extra & !bloss: Extra process rates for MSE budget outputs
                      )
         ...
         pcp_gr(i,j) = pptgraul
         pcp_ic(i,j) = pptice
         RAINNCV(i,j) = pptrain + pptsnow + pptgraul + pptice + pptcliq
         RAINNC(i,j) = RAINNC(i,j) + pptrain + pptsnow + pptgraul + pptice + pptcliq
...
!SAM#if ( WRF_CHEM == 1 )
!SAM            rainprod(i,k,j) = rainprod1d(k)
!SAM            evapprod(i,k,j) = evapprod1d(k)
!SAM#endif
...
!..Sub arguments
      ...
      REAL, INTENT(INOUT):: pptrain, pptsnow, pptgraul, pptice, pptcliq
      REAL, INTENT(IN):: dt
!SAM#if ( WRF_CHEM == 1 )
!SAM      REAL, DIMENSION(kts:kte), INTENT(INOUT):: &
!SAM                          rainprod, evapprod
!SAM#endif

      !bloss: Additional inputs/outputs
      REAL, DIMENSION(kts:kte), INTENT(IN):: rho1d
      REAL, DIMENSION(kts:kte,6), INTENT(OUT) :: mtendq, stendq
      REAL, DIMENSION(kts:kte,5), INTENT(OUT) ::mtendn, stendn
...
!..Local variables
      ...
      DOUBLE PRECISION, DIMENSION(kts:kte):: prg_scw, prg_rfz, prg_gde, &
           prg_gcw, prg_rci, prg_rcs, prg_scr, & !bloss: prg_scr used with doRearrangeRainIceCollection
           prg_rcg, prg_ihm
...
!+---+-----------------------------------------------------------------+
      ...
      do k = kts, kte
         ...
         prg_rci(k) = 0.
         prg_rcs(k) = 0.
         prg_scr(k) = 0. !bloss: Used with doRearrangeRainIceCollection
         prg_rcg(k) = 0.
         prg_ihm(k) = 0.
         prw_iml(k) = 0.
...
!+---+-----------------------------------------------------------------+
      do k = kts, kte
         ...
         pres(k) = p1d(k)
         rho(k) = rho1d(k) !bloss(SAM) 0.622*pres(k)/(R*temp(k)*(qv(k)+0.622))
...
!.. results in lookup table.
         if (rr(k).ge. r_r(1)) then
          if (rs(k).ge. r_s(1)) then
           if(doRearrangeRainIceCollection) then
            ...
            if (temp(k).ge.T_0) then
             ...
             prs_rcs(k) = tcr_sacr2(idx_s,idx_t,idx_r1,idx_r)
             ...
             ! No graupel produced above 0C
             prg_rcs(k) = 0.
             prg_scr(k) = 0.

             ! loss of rain number due to snow collecting rain.
             pnr_rcs(k) = tnr_sacr2(idx_s,idx_t,idx_r1,idx_r)
             pnr_rcs(k) = MIN(DBLE(nr(k)*odts), pnr_rcs(k))

             !bloss(NOTE): Original Thompson code includes mass of 
             !  rain collecting snow and snow collecting rain in these 
             !  terms which seems excessive for these one-way tendencies
             !  if that snow/rain does not melt/freeze instantly.
             !  Thompson also neglects any rain number tendencies above
             !  0C, which seems inconsistent with snow collecting rain.
             !  Since, there is probably little snow above 0C, the impact
             !  of changing these tendencies is probably minimal.

           else  ! if (temp(k).lt.T_0) 
             !bloss: This total tendencies produced by this code
             !  should be identical to those produced by original code
             !  except for tendencies affected by limiting.
             
             ! rain does not gain mass
             prr_rcs(k) = 0.

             ! prs_rcs: rain-snow collisions turn rain into snow
             !   source hydrometeor is rain
             prs_rcs(k) = tmr_racs2(idx_s,idx_t,idx_r1,idx_r) &
                  + tcr_sacr2(idx_s,idx_t,idx_r1,idx_r)
             prs_rcs(k) = MIN(DBLE(rr(k)*odts), prs_rcs(k))

             ! prg_rcs: rain-snow collisions turn snow into graupel
             !   source hydrometeor is snow.
             prg_rcs(k) = tcs_racs1(idx_s,idx_t,idx_r1,idx_r)           &
                  + tms_sacr1(idx_s,idx_t,idx_r1,idx_r)
             prg_rcs(k) = MIN(DBLE(rs(k)*odts), prg_rcs(k))

             ! prg_scr: rain-snow collisions turn rain into graupel
             ...
             ! prg_scr: rain-snow collisions reduce rain number
	   ...
	   !bloss(end): Rearrangement of rain-snow collection
	  else
          ...
          end if !blosss: if(doRearrangeRainIceCollection)
          endif
...
!..Snow conservation.
         if(doRearrangeRainIceCollection) then
           sump = prs_sde(k) - prs_ihm(k) - prr_sml(k) &
                - prr_rcs(k) - prg_rcs(k)  !bloss: prr_rcs, prg_rcs both positive-definite
         else
           sump = prs_sde(k) - prs_ihm(k) - prr_sml(k) &
                + prs_rcs(k) !bloss: prs_sde and prs_rcs can be negative
         endif
         rate_max = -rs(k)*odts
...
!.. any of the above terms were altered.  Thanks P. Blossey. 2009Sep28
	    ...
	    if(doRearrangeRainIceCollection) then
	    !bloss: the below code is not needed when the
	    !  processes are rearranged so that all processes
	    !  have a single source and single destination.
	    !  The limiting (and the isotope implementation)
	    !  is simpler in that case.
else
...
!bloss(TODO): I wonder whether this should be xrc instead of rc(k),
!          because xrc is updated with the new qcten(k).
!bloss(Is this correct?)          lamc = (xnc*am_r*ccg(2,nu_c)*ocg1(nu_c)/xrc)**obmr
          xDc = (bm_r + nu_c + 1.) / lamc
          if (xDc.lt. D0c) then
           lamc = cce(2,nu_c)/D0c
...
!SAM#if ( WRF_CHEM == 1 )
!SAM      do k = kts, kte
!SAM         evapprod(k) = prv_rev(k) - (min(zeroD0,prs_sde(k)) + &
!SAM                                     min(zeroD0,prg_gde(k)))
!SAM         rainprod(k) = prr_wau(k) + prr_rcw(k) + prs_scw(k) + &
!SAM                                    prg_scw(k) + prs_iau(k) + &
!SAM                                    prg_gcw(k) + prs_sci(k) + &
!SAM                                    pri_rci(k)
!SAM      enddo
!SAM#endif
      ...
      mtendn(kts:kte,3) = ncten(kts:kte)
      mtendn(kts:kte,4) = nwfaten(kts:kte)
      mtendn(kts:kte,5) = nifaten(kts:kte)
...
!+---+-----------------------------------------------------------------+
      ...
      stendn(kts:kte,3) = ncten(kts:kte) - mtendn(kts:kte,3) ! cloud liquid number
!+---+-----------------------------------------------------------------+
...
!+---+-----------------------------------------------------------------+
      ...
      if(do_accum_proc_rates) then
        ...
        if(.NOT.iiwarm) then
          ...
          iidx = iidx + 1
          mass_proc_rates(:,iidx) = mass_proc_rates(:,iidx) + pri_iha/rho
          iidx = iidx + 1
        ...
        if(.NOT.iiwarm) then
          ...
          iidx = iidx + 1
          number_proc_rates(:,iidx) = number_proc_rates(:,iidx) + pni_iha/rho ! new ni source/sink
          iidx = iidx + 1
      ...
      if(is_aerosol_aware) then
         iidx = iidx + 1
         number_proc_rates(:,iidx) = number_proc_rates(:,iidx) + pnc_wcd ! nc sources/sinks
         iidx = iidx + 1
         number_proc_rates(:,iidx) = number_proc_rates(:,iidx) + pnc_wau/rho ! nc sources/sinks
         iidx = iidx + 1
         number_proc_rates(:,iidx) = number_proc_rates(:,iidx) + pnc_rcw/rho
         iidx = iidx + 1
         number_proc_rates(:,iidx) = number_proc_rates(:,iidx) + pna_rca/rho ! nwfa scavenging

         if(.NOT.iiwarm) then
            iidx = iidx + 1
            number_proc_rates(:,iidx) = number_proc_rates(:,iidx) + pnc_scw/rho
            iidx = iidx + 1
            number_proc_rates(:,iidx) = number_proc_rates(:,iidx) + pnc_gcw/rho
            iidx = iidx + 1
            number_proc_rates(:,iidx) = number_proc_rates(:,iidx) + pna_sca/rho
            iidx = iidx + 1
            number_proc_rates(:,iidx) = number_proc_rates(:,iidx) + pna_gca/rho
            iidx = iidx + 1
            number_proc_rates(:,iidx) = number_proc_rates(:,iidx) + pnd_rcd/rho ! nifa scavenging
            iidx = iidx + 1
            number_proc_rates(:,iidx) = number_proc_rates(:,iidx) + pnd_scd/rho
            iidx = iidx + 1
            number_proc_rates(:,iidx) = number_proc_rates(:,iidx) + pnd_gcd/rho
         end if ! if(ice microphysics enabled)
      end if ! if(is_aerosol_aware)

   end if !if(do_accum_proc_rates)
...
!+---+-----------------------------------------------------------------+
 subroutine mp_thompson_proc_names(mass_process_names,mass_process_longnames, &
   ...
   if(.NOT.iiwarm) then
      ...
      iidx = iidx + 1
      mass_process_names(iidx) = 'pri_iha'
      mass_process_longnames(iidx) = 'pri_iha: cloud ice mass source due to freezing of aqueous aerosol'
      iidx = iidx + 1
   ...
   if(.NOT.iiwarm) then
      ...
      iidx = iidx + 1
      number_process_names(iidx) = 'pni_iha'
      number_process_longnames(iidx) = 'pni_iha: cloud ice mass source due to freezing of aqueous aerosol'
      iidx = iidx + 1
   ...
   if(is_aerosol_aware) then
      iidx = iidx + 1
      number_process_names(iidx) = 'pnc_wcd' ! nc sources/sinks
      number_process_longnames(iidx) = 'pnc_wcd: cloud droplet number source due to activation'
      iidx = iidx + 1
      number_process_names(iidx) = 'pnc_wau' ! nc sources/sinks
      number_process_longnames(iidx) = 'pnc_wau: cloud droplet number sink due to autoconversion'
      iidx = iidx + 1
      number_process_names(iidx) = 'pnc_rcw'
      number_process_longnames(iidx) = 'pnc_rcw: cloud droplet number sink due to collection by rain'
      iidx = iidx + 1
      number_process_names(iidx) = 'pna_rca' ! nwfa scavenging
      number_process_longnames(iidx) = 'pna_rca: water-friendly aerosol scavenging by rain'

      if(.NOT.iiwarm) then
         iidx = iidx + 1
         number_process_names(iidx) = 'pnc_scw'
         number_process_longnames(iidx) = 'pnc_scw: cloud droplet sink due to collection by snow'
         iidx = iidx + 1
         number_process_names(iidx) = 'pnc_gcw'
         number_process_longnames(iidx) = 'pnc_gcw: cloud droplet sink due to collection by graupel'
         iidx = iidx + 1
         number_process_names(iidx) = 'pna_sca'
         number_process_longnames(iidx) = 'pna_sca: water-friendly aerosol scavenging by snow'
         iidx = iidx + 1
         number_process_names(iidx) = 'pna_gca'
         number_process_longnames(iidx) = 'pna_gca: water-friendly aerosol scavenging by graupel'
         iidx = iidx + 1
         number_process_names(iidx) = 'pnd_rcd' ! nifa scavenging
         number_process_longnames(iidx) = 'pnd_rcd: ice-friendly aerosol sink due to scavenging by rain'
         iidx = iidx + 1
         number_process_names(iidx) = 'pnd_scd'
         number_process_longnames(iidx) = 'pnd_scd: ice-friendly aerosol sink due to scavenging by snow'
         iidx = iidx + 1
         number_process_names(iidx) = 'pnd_gcd'
         number_process_longnames(iidx) = 'pnd_gcd: ice-friendly aerosol sink due to scavenging by graupel'
      end if ! if(ice microphysics enabled)
   end if ! if(is_aerosol_aware)
...
!..Local variables
      ...
      LOGICAL :: force_read_thompson = .false., write_thompson_tables = .true. !SAM
      LOGICAL lexist,lopen
      INTEGER good, itmp(1) !SAM
!SAM      LOGICAL, EXTERNAL :: wrf_dm_on_monitor

!+---+

      !SAM: These calls read a namelist variable in WRF that requires the lookup table to be 
      !   read from a file.
!SAM      CALL nl_get_force_read_thompson(1,force_read_thompson)
!SAM      CALL nl_get_write_thompson_tables(1,write_thompson_tables)

      good = 0
!SAM      IF ( wrf_dm_on_monitor() ) THEN
      if(masterproc) then
        INQUIRE(FILE="RUNDATA/qr_acr_qgV2.dat",EXIST=lexist)
        IF ( lexist ) THEN
          CALL wrf_message("ThompMP: read RUNDATA/qr_acr_qgV2.dat instead of computing")
          OPEN(63,file="RUNDATA/qr_acr_qgV2.dat",form="unformatted",err=1234)
 ...
 1234     CONTINUE
          IF ( good .NE. 1 ) THEN
            ...
            IF (lopen) THEN
              IF( force_read_thompson ) THEN
                CALL wrf_error_fatal("Error reading RUNDATA/qr_acr_qgV2.dat. Aborting because force_read_thompson is .true.")
              ENDIF
              CLOSE(63)
            ELSE
              IF( force_read_thompson ) THEN
                CALL wrf_error_fatal("Error opening RUNDATA/qr_acr_qgV2.dat. Aborting because force_read_thompson is .true.")
              ENDIF
            ENDIF
          ELSE
            INQUIRE(63,OPENED=lopen)
        ...
        ELSE
          IF( force_read_thompson ) THEN
            CALL wrf_error_fatal("Non-existent RUNDATA/qr_acr_qgV2.dat. Aborting because force_read_thompson is .true.")
          ENDIF
        ENDIF
      ENDIF
!SAM#if defined(DM_PARALLEL) && !defined(STUBMPI)
!SAM      CALL wrf_dm_bcast_integer(good,1)
!SAM#endif
!SAM -- begin
      if(dompi) then 
        itmp(1) = 0
        if(masterproc) itmp(1) = good
...
!SAM -- end
      ...
      IF ( good .EQ. 1 ) THEN
!SAM#if defined(DM_PARALLEL) && !defined(STUBMPI)
!SAM        CALL wrf_dm_bcast_double(tcg_racg,SIZE(tcg_racg))
!SAM        CALL wrf_dm_bcast_double(tmr_racg,SIZE(tmr_racg))
!SAM        CALL wrf_dm_bcast_double(tcr_gacr,SIZE(tcr_gacr))
!SAM        CALL wrf_dm_bcast_double(tmg_gacr,SIZE(tmg_gacr))
!SAM        CALL wrf_dm_bcast_double(tnr_racg,SIZE(tnr_racg))
!SAM        CALL wrf_dm_bcast_double(tnr_gacr,SIZE(tnr_gacr))
!SAM#endif

!SAM -- begin
        if(dompi) then
          call task_bcast_fourdim_array_real8(0,tcg_racg) !bloss ,SIZE(tcg_racg))
          CALL task_bcast_fourdim_array_real8(0,tmr_racg) !bloss ,SIZE(tmr_racg))
          CALL task_bcast_fourdim_array_real8(0,tcr_gacr) !bloss ,SIZE(tcr_gacr))
          CALL task_bcast_fourdim_array_real8(0,tmg_gacr) !bloss ,SIZE(tmg_gacr))
          CALL task_bcast_fourdim_array_real8(0,tnr_racg) !bloss ,SIZE(tnr_racg))
          CALL task_bcast_fourdim_array_real8(0,tnr_gacr) !bloss ,SIZE(tnr_gacr))
        end if
!SAM -- end

      ELSE
        if(masterproc) CALL wrf_message("ThompMP: computing qr_acr_qg")
        do n2 = 1, nbr
...
!SAM#if ( defined( DM_PARALLEL ) && ( ! defined( STUBMPI ) ) )
        if(dompi) then
	  !SAM(bloss): Use double precision gatherv routine directly.
          CALL wrf_dm_gatherv_double(tcg_racg, ntb_g*ntb_g1, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tmr_racg, ntb_g*ntb_g1, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tcr_gacr, ntb_g*ntb_g1, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tmg_gacr, ntb_g*ntb_g1, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tnr_racg, ntb_g*ntb_g1, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tnr_gacr, ntb_g*ntb_g1, km_s, km_e) !SAM , R8SIZE)
        end if
!SAM#endif

!SAM        IF ( write_thompson_tables .AND. wrf_dm_on_monitor() ) THEN
        if(write_thompson_tables.AND.masterproc) then
          CALL wrf_message("Writing RUNDATA/qr_acr_qgV2.dat in Thompson MP init")
          OPEN(63,file="RUNDATA/qr_acr_qgV2.dat",form="unformatted",err=9234)
          ...
          RETURN    ! ----- RETURN
 9234     CONTINUE
          CALL wrf_error_fatal("Error writing RUNDATA/qr_acr_qgV2.dat")
        ENDIF
      ENDIF
...
!SAM      IF ( wrf_dm_on_monitor() ) THEN
      if(masterproc) then
        INQUIRE(FILE="RUNDATA/qr_acr_qsV2.dat",EXIST=lexist)
        IF ( lexist ) THEN
          CALL wrf_message("ThompMP: read RUNDATA/qr_acr_qsV2.dat instead of computing")
          OPEN(63,file="RUNDATA/qr_acr_qsV2.dat",form="unformatted",err=1234)
 ...
 1234     CONTINUE
          IF ( good .NE. 1 ) THEN
            ...
            IF (lopen) THEN
              IF( force_read_thompson ) THEN
                CALL wrf_error_fatal("Error reading RUNDATA/qr_acr_qsV2.dat. Aborting because force_read_thompson is .true.")
              ENDIF
              CLOSE(63)
            ELSE
              IF( force_read_thompson ) THEN
                CALL wrf_error_fatal("Error opening RUNDATA/qr_acr_qsV2.dat. Aborting because force_read_thompson is .true.")
              ENDIF
            ENDIF
          ELSE
            INQUIRE(63,opened=lopen)
        ...
        ELSE
          IF( force_read_thompson ) THEN
            CALL wrf_error_fatal("Non-existent RUNDATA/qr_acr_qsV2.dat. Aborting because force_read_thompson is .true.")
          ENDIF
        ENDIF
      ENDIF
!SAM#if defined(DM_PARALLEL) && !defined(STUBMPI)
...
!SAM -- end
      ...
      IF ( good .EQ. 1 ) THEN
!SAM#if defined(DM_PARALLEL) && !defined(STUBMPI)
!SAM        CALL wrf_dm_bcast_double(tcs_racs1,SIZE(tcs_racs1))
!SAM        CALL wrf_dm_bcast_double(tmr_racs1,SIZE(tmr_racs1))
!SAM        CALL wrf_dm_bcast_double(tcs_racs2,SIZE(tcs_racs2))
!SAM        CALL wrf_dm_bcast_double(tmr_racs2,SIZE(tmr_racs2))
!SAM        CALL wrf_dm_bcast_double(tcr_sacr1,SIZE(tcr_sacr1))
!SAM        CALL wrf_dm_bcast_double(tms_sacr1,SIZE(tms_sacr1))
!SAM        CALL wrf_dm_bcast_double(tcr_sacr2,SIZE(tcr_sacr2))
!SAM        CALL wrf_dm_bcast_double(tms_sacr2,SIZE(tms_sacr2))
!SAM        CALL wrf_dm_bcast_double(tnr_racs1,SIZE(tnr_racs1))
!SAM        CALL wrf_dm_bcast_double(tnr_racs2,SIZE(tnr_racs2))
!SAM        CALL wrf_dm_bcast_double(tnr_sacr1,SIZE(tnr_sacr1))
!SAM        CALL wrf_dm_bcast_double(tnr_sacr2,SIZE(tnr_sacr2))
!SAM#endif

!SAM -- begin
        if(dompi) then
          CALL task_bcast_fourdim_array_real8(0,tcs_racs1) !bloss ,SIZE(tcs_racs1))
          CALL task_bcast_fourdim_array_real8(0,tmr_racs1) !bloss ,SIZE(tmr_racs1))
          CALL task_bcast_fourdim_array_real8(0,tcs_racs2) !bloss ,SIZE(tcs_racs2))
          CALL task_bcast_fourdim_array_real8(0,tmr_racs2) !bloss ,SIZE(tmr_racs2))
          CALL task_bcast_fourdim_array_real8(0,tcr_sacr1) !bloss ,SIZE(tcr_sacr1))
          CALL task_bcast_fourdim_array_real8(0,tms_sacr1) !bloss ,SIZE(tms_sacr1))
          CALL task_bcast_fourdim_array_real8(0,tcr_sacr2) !bloss ,SIZE(tcr_sacr2))
          CALL task_bcast_fourdim_array_real8(0,tms_sacr2) !bloss ,SIZE(tms_sacr2))
          CALL task_bcast_fourdim_array_real8(0,tnr_racs1) !bloss ,SIZE(tnr_racs1))
          CALL task_bcast_fourdim_array_real8(0,tnr_racs2) !bloss ,SIZE(tnr_racs2))
          CALL task_bcast_fourdim_array_real8(0,tnr_sacr1) !bloss ,SIZE(tnr_sacr1))
          CALL task_bcast_fourdim_array_real8(0,tnr_sacr2) !bloss ,SIZE(tnr_sacr2))
        end if
!SAM -- end

      ELSE
        if(masterproc) CALL wrf_message("ThompMP: computing qr_acr_qs")
        do n2 = 1, nbr
!        vr(n2) = av_r*Dr(n2)**bv_r * DEXP(-fv_r*Dr(n2))
         vr(n2) = -0.1021 + 4.932E3*Dr(n2) - 0.9551E6*Dr(n2)*Dr(n2)     &
              + 0.07934E9*Dr(n2)*Dr(n2)*Dr(n2)                          &
...
!SAM#if ( defined( DM_PARALLEL ) && ( ! defined( STUBMPI ) ) )
        if(dompi) then
	  !SAM(bloss): Use double precision gatherv routine directly.
          CALL wrf_dm_gatherv_double(tcs_racs1, ntb_s*ntb_t, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tmr_racs1, ntb_s*ntb_t, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tcs_racs2, ntb_s*ntb_t, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tmr_racs2, ntb_s*ntb_t, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tcr_sacr1, ntb_s*ntb_t, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tms_sacr1, ntb_s*ntb_t, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tcr_sacr2, ntb_s*ntb_t, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tms_sacr2, ntb_s*ntb_t, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tnr_racs1, ntb_s*ntb_t, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tnr_racs2, ntb_s*ntb_t, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tnr_sacr1, ntb_s*ntb_t, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tnr_sacr2, ntb_s*ntb_t, km_s, km_e) !SAM , R8SIZE)
        end if
!SAM#endif

!SAM        IF ( write_thompson_tables .AND. wrf_dm_on_monitor() ) THEN
        IF ( write_thompson_tables .AND. masterproc ) THEN
          CALL wrf_message("Writing RUNDATA/qr_acr_qsV2.dat in Thompson MP init")
          OPEN(63,file="RUNDATA/qr_acr_qsV2.dat",form="unformatted",err=9234)
          ...
          RETURN    ! ----- RETURN
 9234     CONTINUE
          CALL wrf_error_fatal("Error writing RUNDATA/qr_acr_qsV2.dat")
        ENDIF
      ENDIF

      if(masterproc) then
        write(*,*)
        write(*,*) 'THOMPSON-EIDHAMMER SNOW-RAIN COLLISION LOOKUP TABLES from qr_acr_qs'
        WRITE(*,*) ' MAX/MIN tcs_racs1 = ',MAXVAL(tcs_racs1), MINVAL(tcs_racs1)
        WRITE(*,*) ' MAX/MIN tmr_racs1 = ',MAXVAL(tmr_racs1), MINVAL(tmr_racs1)
        WRITE(*,*) ' MAX/MIN tcs_racs2 = ',MAXVAL(tcs_racs2), MINVAL(tcs_racs2)
        WRITE(*,*) ' MAX/MIN tmr_racs2 = ',MAXVAL(tmr_racs2), MINVAL(tmr_racs2)
        WRITE(*,*) ' MAX/MIN tcr_sacr1 = ',MAXVAL(tcr_sacr1), MINVAL(tcr_sacr1)
        WRITE(*,*) ' MAX/MIN tms_sacr1 = ',MAXVAL(tms_sacr1), MINVAL(tms_sacr1)
        WRITE(*,*) ' MAX/MIN tcr_sacr2 = ',MAXVAL(tcr_sacr2), MINVAL(tcr_sacr2)
        WRITE(*,*) ' MAX/MIN tms_sacr2 = ',MAXVAL(tms_sacr2), MINVAL(tms_sacr2)
        WRITE(*,*) ' MAX/MIN tnr_racs1 = ',MAXVAL(tnr_racs1), MINVAL(tnr_racs1)
        WRITE(*,*) ' MAX/MIN tnr_racs2 = ',MAXVAL(tnr_racs2), MINVAL(tnr_racs2)
        WRITE(*,*) ' MAX/MIN tnr_sacr1 = ',MAXVAL(tnr_sacr1), MINVAL(tnr_sacr1)
        WRITE(*,*) ' MAX/MIN tnr_sacr2 = ',MAXVAL(tnr_sacr2), MINVAL(tnr_sacr2)
        write(*,*)
      end if
...
!SAM      IF ( wrf_dm_on_monitor() ) THEN
      if(masterproc) then
        INQUIRE(FILE="RUNDATA/freezeH2O.dat",EXIST=lexist)
        IF ( lexist ) THEN
          CALL wrf_message("ThompMP: read freezeH2O.dat instead of computing")
          OPEN(63,file="RUNDATA/freezeH2O.dat",form="unformatted",err=1234)
...
!SAM -- end
      ...
      IF ( good .EQ. 1 ) THEN
!SAM#if defined(DM_PARALLEL) && !defined(STUBMPI)
!SAM        CALL wrf_dm_bcast_double(tpi_qrfz,SIZE(tpi_qrfz))
!SAM        CALL wrf_dm_bcast_double(tni_qrfz,SIZE(tni_qrfz))
!SAM        CALL wrf_dm_bcast_double(tpg_qrfz,SIZE(tpg_qrfz))
!SAM        CALL wrf_dm_bcast_double(tnr_qrfz,SIZE(tnr_qrfz))
!SAM        CALL wrf_dm_bcast_double(tpi_qcfz,SIZE(tpi_qcfz))
!SAM        CALL wrf_dm_bcast_double(tni_qcfz,SIZE(tni_qcfz))
!SAM#endif

!SAM -- begin
        if(dompi) then
          CALL task_bcast_fourdim_array_real8(0,tpi_qrfz) !bloss ,SIZE(tpi_qrfz))
          CALL task_bcast_fourdim_array_real8(0,tni_qrfz) !bloss ,SIZE(tni_qrfz))
          CALL task_bcast_fourdim_array_real8(0,tpg_qrfz) !bloss ,SIZE(tpg_qrfz))
          CALL task_bcast_fourdim_array_real8(0,tnr_qrfz) !bloss ,SIZE(tnr_qrfz))
          CALL task_bcast_fourdim_array_real8(0,tpi_qcfz) !bloss ,SIZE(tpi_qcfz))
          CALL task_bcast_fourdim_array_real8(0,tni_qcfz) !bloss ,SIZE(tni_qcfz))
        end if
!SAM -- end

      ELSE
        if(masterproc) CALL wrf_message("ThompMP: computing freezeH2O")
...
!SAM#ifdef THOMPSON_OMP
!SAM!$OMP PARALLEL DO SCHEDULE(dynamic) &
!SAM!$OMP PRIVATE(j,i,lam_exp,lamr,N0_r,sum1,sum2,sumn1,sumn2,n2,N_r,vol,prob)
!SAM#endif
         do j = 1, ntb_r1
            do i = 1, ntb_r
...
!SAM!$OMP PARALLEL DO SCHEDULE(dynamic) &
!SAM!$OMP PRIVATE(j,i,nu_c,lamc,N0_c,sum1,sumn2,vol,prob,N_c)
!SAM#endif
         do j = 1, nbc
...
!SAM#if ( defined( DM_PARALLEL ) && ( ! defined( STUBMPI ) ) )
        if(dompi) then
	  !SAM(bloss): Use double precision gatherv routine directly.
          CALL wrf_dm_gatherv_double(tpi_qrfz, ntb_r*ntb_r1, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tni_qrfz, ntb_r*ntb_r1, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tpg_qrfz, ntb_r*ntb_r1, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tnr_qrfz, ntb_r*ntb_r1, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tpi_qcfz, ntb_c*nbc, km_s, km_e) !SAM , R8SIZE)
          CALL wrf_dm_gatherv_double(tni_qcfz, ntb_c*nbc, km_s, km_e) !SAM , R8SIZE)
        end if
!SAM#endif
...
!SAM        IF ( write_thompson_tables .AND. wrf_dm_on_monitor() ) THEN
        IF ( write_thompson_tables .AND. masterproc ) THEN
          ...
          OPEN(63,file="RUNDATA/freezeH2O.dat",form="unformatted",err=9234)
...
!SAM      IF ( wrf_dm_on_monitor() ) THEN
 ...
 2010   CONTINUE
      ENDIF
!SAM#if defined(DM_PARALLEL) && !defined(STUBMPI)
!SAM      CALL wrf_dm_bcast_bytes ( iunit_mp_th1 , IWORDSIZE )
      if(dompi) then
        itmp(1) = 0
        if(masterproc) itmp(1) = iunit_mp_th1
        call task_bcast_integer(0,itmp,1)
        iunit_mp_th1 = itmp(1)
      end if
!SAM#endif
...
!SAM      IF ( wrf_dm_on_monitor() ) THEN
      IF (masterproc) THEN
        ...
        OPEN(iunit_mp_th1,FILE='RUNDATA/CCN_ACTIVATE.BIN',                      &
             FORM='UNFORMATTED',STATUS='OLD',CONVERT="BIG_ENDIAN")
      ENDIF

!SAM#define DM_BCAST_MACRO(A) CALL wrf_dm_bcast_bytes(A, size(A)*R4SIZE)

!SAM      IF ( wrf_dm_on_monitor() ) READ(iunit_mp_th1,ERR=9010) tnccn_act
!SAM#if defined(DM_PARALLEL) && !defined(STUBMPI)
!SAM      DM_BCAST_MACRO(tnccn_act)
!SAM#endif

      if(masterproc) then
        READ(iunit_mp_th1,ERR=9010) tnccn_act
        CLOSE(iunit_mp_th1)
        write(*,*) 'MIN/MAX(tnccn_act) = ', MINVAL(tnccn_act), MAXVAL(tnccn_act)
        if(MAXVAL(tnccn_act).gt.1.e3) then
          write(*,*) 'Bad value in activation lookup table, stopping...'
          call task_abort()
        end if
      end if
      if(dompi) then
        call task_bcast_fivedim_array_real4(0,tnccn_act)
      end if
...
!     --- USES GAMMLN
      ...
      D=1./B
      H=D
      !bloss: change to do/enddo from do/continue
      DO I=1,ITMAX ! DO 11 I=1,ITMAX
        AN=-I*(I-A)
        B=B+2.
        D=AN*D+B
...
!     --- RETURNS THE VALUE LN(GAMMA(XX)) FOR XX > 0.
      ...
      !bloss: Use do/enddo instead of do/continue
      DO J=1,6  !bloss      DO 11 J=1,6
        Y=Y+1.D0
        SER=SER+COF(J)/Y
      END DO    !bloss 11    CONTINUE
...
!
      ...
      REAL, EXTERNAL :: qsatw !bloss: SAM built-in function

      !bloss: Use SAM built-in saturation vapor mixing ratio function.
      !  Extends to colder temperatures.
      RSLF=qsatw(T,P/100.) 

!bloss/SAM      X=MAX(-80.,T-273.16)
!bloss/SAM
!bloss/SAM!      ESL=612.2*EXP(17.67*X/(T-29.65))
!bloss/SAM      ESL=C0+X*(C1+X*(C2+X*(C3+X*(C4+X*(C5+X*(C6+X*(C7+X*C8)))))))
!bloss/SAM      ESL=MIN(ESL, P*0.15)        ! Even with P=1050mb and T=55C, the sat. vap. pres only contributes to ~15% of total pres.
!bloss/SAM      RSLF=.622*ESL/(P-ESL)
...
!
      ...
      REAL, EXTERNAL :: qsati !bloss: SAM built-in function
...
!bloss/SAM      X=MAX(-80.,T-273.16)
!bloss/SAM      ESI=C0+X*(C1+X*(C2+X*(C3+X*(C4+X*(C5+X*(C6+X*(C7+X*C8)))))))
!bloss/SAM      ESI=MIN(ESI, P*0.15)
!bloss/SAM      RSIF=.622*ESI/(P-ESI)
...
!+---+-----------------------------------------------------------------+
    END MODULE module_mp_thompson_iso_AerosolAware
!+---+-----------------------------------------------------------------+
