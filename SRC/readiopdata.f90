...
!     
   ...
   !bloss(2023-12): Check surface large-scale vertical velocity
   real :: wls_surf
   real, parameter :: wls_surf_threshold = 1.e-3
...
!     
   ...
   if(have_wls) then
      ! consistency check: wls(z=0) should be zero (or at least close to it)
      coef = (Ps_in(1)-dplevs(nlev-1))/(dplevs(nlev)-dplevs(nlev-1))
      wls_surf = coef*wls_in(1,nlev)+(1.-coef)*wls_in(1,nlev-1)
      if(masterproc) write(*,*) 'Surface wls = ',wls_surf,' m/s at z=0'
      if(ABS(coef*wls_in(1,nlev)+(1.-coef)*wls_in(1,nlev-1)).gt.wls_surf_threshold) then
         if(masterproc) write(*,*) 'Surface w_ls is inconsistent with bottom BC (i.e., w(z=0) = 0).'
         call task_abort()
      end if
   end if
   ...
   if(get_add_surface_data) then
      ...
      ! large-scale vertical velocity is zero at surface
      if(have_wls) wls_in(:,nlev+1) = 0.
