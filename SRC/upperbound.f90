...
use vars
use params
use microphysics, only: micro_field, index_water_vapor, is_water_vapor, nmicro_fields, mk0, mklsadv, mkobs, mknudge, flag_nudging
implicit none
real coef, coefxy
integer i,j,k,n
...
!
  ...
  do n = 1,nmicro_fields
    if((n.ne.index_water_vapor).AND.(flag_nudging(n).eq.1)) then
      !bloss: Nudge fields like isotopic water vapor or aerosols in adjacent to upper boundary
      coef = dtn / tau_nudging
      do k=nzm-1,nzm
        do j=1,ny
          do i=1,nx
            !bloss: add tendency to *NUDGE so that it is included in the
            !  vertically-integrated energy and moisture budgets.
             dq = - (micro_field(i,j,k,n)-mkobs(k,n))*coef
             mknudge(k,n) = mknudge(k,n) + dq*coefxy
            micro_field(i,j,k,n) = micro_field(i,j,k,n) + dq
          end do
        end do
      end do
    end if !if(flag_nudging...
  end do
    ...
    if( is_water_vapor(n) .AND. (n.ne.index_water_vapor) ) then
      ...
      do j=1,ny
        do i=1,nx
          !bloss: add tendency to *NUDGE so that it is included in the
          !  vertically-integrated energy and moisture budgets.
          mknudge(nzm,n) = mknudge(nzm,n) + coef*( gamq0 - micro_field(i,j,nzm,n) )*coefxy
